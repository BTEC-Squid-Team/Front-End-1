...
class Profile extends React.Component {
  ...
  addBio = async (e) => {
    ...
    let newBioData = await axios.post(
      `${process.env.REACT_APP_SERVER}/addBio`,
      bioFormInfo
    );
  ...
  componentDidMount = async () => {
    ...
    let bioData = await axios.get(
      `${process.env.REACT_APP_SERVER}/getBio?email=${email}`
    );
  ...
  updateBio = async (e) => {
    ...
    let newBioData = await axios.put(
      `${process.env.REACT_APP_SERVER}/updateBio`,
      bioFormInfo
    );
    this.setState({
      bios: newBioData.data,
  ...
  render() {
    return (
      <div>
        ...
        <p class="userInfo" id="username">
          {this.props.auth0.user.name}{" "}
        </p>
        <p class="userInfo" id="email">
          {this.props.auth0.user.email}
        </p>

        <AddBioForm addBioFunc={this.addBio} />

        {this.state.showBiosComponent &&
          this.state.bios.map((bio) => {
            return <Bio showUpdateBioForm={this.showUpdateBioForm} bio={bio} />;
          })}
